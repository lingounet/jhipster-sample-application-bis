...
public class PsatService {
    ...
    /**
     *  Get all the psats where Identity is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<Psat> findAllWhereIdentityIsNull() {
        log.debug("Request to get all psats where Identity is null");
        return StreamSupport
            .stream(psatRepository.findAll().spliterator(), false)
            .filter(psat -> psat.getIdentity() == null)
