...
public class SecurityInterviewService {
    ...
    /**
     *  Get all the securityInterviews where ApplicationType is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<SecurityInterview> findAllWhereApplicationTypeIsNull() {
        log.debug("Request to get all securityInterviews where ApplicationType is null");
        return StreamSupport
            .stream(securityInterviewRepository.findAll().spliterator(), false)
            .filter(securityInterview -> securityInterview.getApplicationType() == null)
