...
class SecurityInterviewResourceIT {
    ...
    void checkApplicationNameIsRequired() throws Exception {
        int databaseSizeBeforeTest = securityInterviewRepository.findAll().size();
        // set the field null
        securityInterview.setApplicationName(null);

        // Create the SecurityInterview, which fails.
        ...
        List<SecurityInterview> securityInterviewList = securityInterviewRepository.findAll();
        assertThat(securityInterviewList).hasSize(databaseSizeBeforeTest);
    }
